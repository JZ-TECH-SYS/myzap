...
function zipDirectory(dir, destZip) {
  ...
  if (process.platform === 'win32') {
    if (run('tar', ['-a', '-cf', destZip, '-C', dir, '.'], { shell: true })) return;
...
/** Copia o codigo RASTREADO pelo git (git archive): nada de node_modules,
 *  dist, lixo local — o pack nasce do que esta commitado. */
function exportSource() {
  fs.mkdirSync(pkgDir, { recursive: true });
  const tarPath = path.join(workDir, 'src.tar');
  const ok = run('git', ['archive', '--format=tar', '-o', tarPath, 'HEAD'], { cwd: repoRoot });
  if (!ok) throw new Error('git archive falhou');
  if (!run('tar', ['-xf', tarPath, '-C', pkgDir], { shell: process.platform === 'win32' })) {
    throw new Error('extracao do git archive falhou');
  }
